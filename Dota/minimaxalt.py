    ...
    def minimax(self, node, depth, player):
        ...
        if player == "Radiant":
            ...
            for move in node.availableMoves():
                ...
                moveValue = self.minimax(node, depth-1, "Dire")
               # print("WHY")
        ...
        if player == "Dire":
            ...
            for move in node.availableMoves():
                node.makeMove(move, player)
                moveValue = self.minimax(node, depth-1, "Radiant")
               # node.makeMove(move, 0)
...
def makeFirst(game):
     while game.gameOver() == False:
        ...
        game.makeMove(dAI, "Radiant")
        #game.show()

        for i in game.getRadiant():
            print( "R: " + str(i.id))
        print("")
        for i in game.getDire():
            print("D: " + str(i.id))
